...
def store_anagrams(d):
    '''Store anagrams dictionary in a shelf
    '''
    anagrams_shelf = dbm.open('anagrams', 'c')
    for key in d:
        s = pickle.dumps(d[key])
        anagrams_shelf[key]= s
    print("Anagrams stored")
    anagrams_shelf.close()
    
def read_anagrams(word):
    ''' Retrieves and prints a dictionary of anagrams
    '''
    anagrams_shelf = dbm.open('anagrams', 'r')
   
    sig = signature(word)
    try:
        #print(anagrams_shelf[sig])
        return pickle.loads(anagrams_shelf[sig])
    except KeyError:
        return []
